...
2. EAG.m refers to the extra anchored gradient (EAG) algorithms [<sup>1</sup>](#refer1). EAG-C refers to EAG with constant step-size $\alpha = 0.01$, and EAG-V refers to EAG with varying step-size $\alpha_k \in [\frac{0.437}{L}, \frac{0.618}{L}]$, where $L=40$ is a smooth parameter.

3. FEG.m refers to the fast extragradient algorithm[<sup>2</sup>](#refer2), and we set $\tau_{-1} = 0.2; \eta_0 = 0.8; \delta = 0.3;$.

4. EGDA.m is our algorithm.  We set the step-size $\alpha = 0.1$, and  $\tau_0 = 0.1, \tau_1 = 0.9$. 
