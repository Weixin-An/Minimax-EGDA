clc; close all
clear
%% function plot f(x,y) = log(1 + exp(x)) + 3 * x * y - log(1 + exp(y))
[X, Y, Z] = my_fun_plot();
figure(1)
mesh(X,Y,Z)
view(-21,22)
...
hold on
% saddle point plot
saddle = [0.1518; -0.1793];
f = plot3(saddle(1), saddle(2), my_fun(saddle(1), saddle(2)), 'r*', 'LineWidth', 2);
text(saddle(1),saddle(2),my_fun(saddle(1), saddle(2))+40, char('saddle point'));
saveas(f, '.\fig_results\problem.fig');
%% Initial point and iterations
x0 = 5; y0 = 5; maxiter = 1000; addpath('algorithms')
%% GDA
